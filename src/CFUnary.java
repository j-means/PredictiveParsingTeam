...
public class CFUnary extends CFExp {
    ...
    * 
    * else if there is a variable in the expression tree with no entry in env throw
    * an exception with message
    * 
    * "error in eval : variable " + s + " is not bound in the environment"
    * 
    * where s is the variable name (that will be accomplished in the recursive
    * call)
    * 
    * Otherwise, return the value of the expression obtained by using the values
    * given in env and the expression and the CofinFin complement method.
    * 
    **********************************************************************/

   public CofinFin eval(Map<String, CofinFin> env) throws Exception {
      if (env == null) {
         throw new Exception("error in eval : eval is null");
      }
      CofinFin newSub = subExp.eval(env);
      return newSub.complement();
    ...
    * 
    * otherwise return a CFUnary object that is constructed from this one and the
    * result of the recursive call on substitute for the subexpression subExp
    * 
    * Because the old value does not need to be preserved, it is adequate to
    * replace subExp with its result from substitute, and return this.
    * 
    **********************************************************************/
   ...
   public CFExp substitute(Map<String, CFExp> bindings) throws Exception {
      ...
      subExp = subExp.substitute(bindings);
      return this;
